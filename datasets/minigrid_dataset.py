...
class MiniGridDataset(Dataset):
    ...
    def _compute_normalization_stats(self):
        """Compute normalization statistics across all chunks."""
        print("Computing normalization statistics...")
        
        # Collect samples for normalization
        obs_samples = []
        act_samples = []
        ...
        for chunk_idx in range(self.n_chunks):
            ...
            with np.load(chunk_path) as data:
                chunk_size = len(data['observations'])
                for i in range(chunk_size):
                    if len(obs_samples) >= min(1000, self.n_rollout):  # Sample up to 1000 for stats
                        break
                    obs_samples.append(data['observations'][i].flatten())
                    act_samples.append(data['actions'][i])
        
        # Compute statistics
        obs_samples = torch.tensor(np.array(obs_samples))
        act_samples = torch.tensor(np.array(act_samples))
        
        self.obs_mean = torch.mean(obs_samples, dim=0)
        self.obs_std = torch.std(obs_samples, dim=0)
        self.action_mean = torch.mean(act_samples, dim=0)
        self.action_std = torch.std(act_samples, dim=0)
        
        # Handle zero std
        self.obs_std = torch.clamp(self.obs_std, min=1e-8)
        self.action_std = torch.clamp(self.action_std, min=1e-8)
