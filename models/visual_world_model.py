...
class VWorldModelLong(nn.Module):
    def __init__(
        self,
        image_size,  # 224
        num_hist,
        ...
        self.concat_dim = concat_dim  # 0 or 1
        assert (
            concat_dim == 0 or concat_dim == 1
        ), f"concat_dim {concat_dim} not supported."
        print("Model emb_dim: ", self.emb_dim)
    ...
    def decode_obs(self, z_obs):
        ...
        obs = {
            ...
            "proprio": z_obs[
                "proprio"
            ],  # Note: no decoder for proprio for now!
        }
        return obs, diff
    ...
    def separate_emb(self, z):
        ...
        if self.concat_dim == 0:
            ...
            # remove tiled dimensions
            z_proprio = z_proprio[
                :, :, 0, : self.proprio_dim // self.num_proprio_repeat
            ]
            z_act = z_act[:, :, 0, : self.action_dim // self.num_action_repeat]
        z_obs = {"visual": z_visual, "proprio": z_proprio}
        return z_obs, z_act
