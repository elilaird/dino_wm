...
class VWorldModelFullSequence(nn.Module):
    def __init__(
        ...
        train_predictor=False,
        train_decoder=True,
        prediction_offset=1,  # how many steps ahead to predict
    ):
        super().__init__()
    ...
    def predict_sliding_window(self, z, window_size=None):
        """
        Apply sliding window prediction over full sequence
        input : z: (b, num_frames, num_patches, emb_dim)
        ...
        for start_idx in range(num_frames - window_size + 1):
            ...
            window_z = z[:, start_idx:end_idx, :, :]  # (b, window_size, num_patches, emb_dim)
            
            # Reshape for predictor
            window_z_flat = rearrange(window_z, "b t p d -> b (t p) d")
            window_pred_flat = self.predictor(window_z_flat)
            window_pred = rearrange(window_pred_flat, "b (t p) d -> b t p d", t=window_size)
            
            # Store predictions (only for positions that have full context)
            if start_idx == 0:
                # First window: use all predictions
                z_pred[:, start_idx:end_idx, :, :] = window_pred
            else:
                # Subsequent windows: only use the last prediction to avoid overlap
                z_pred[:, end_idx-1:end_idx, :, :] = window_pred[:, -1:, :, :]
    ...
    def forward(self, obs, act):
        ...
        if self.predictor is not None:
            ...
            # Apply sliding window prediction
            z_pred = self.predict_sliding_window(z_src)
            
            if self.decoder is not None:
                obs_pred, diff_pred = self.decode(z_pred.detach())
                visual_pred = obs_pred["visual"]
                visual_tgt = obs["visual"][:, self.prediction_offset:, ...]
...
class VWorldModelWithMemory(VWorldModelFullSequence):
    def __init__(self, mem, memory_variant="mac_shared", *args, **kwargs):
    ...
    def predict_sliding_window(self, z, window_size=None):
        ...
        for start_idx in range(num_frames - window_size + 1):
            ...
            window_z = z[
                :, start_idx:end_idx, :, :
            ]  # (b, window_size, num_patches, emb_dim)

            # INNER LOOP: update memory for window
            ...
            if start_idx == 0:
                ...
                # Subsequent windows: only use the last prediction to avoid overlap
                z_pred[:, end_idx - 1 : end_idx, :, :] = window_pred[
                    :, -1:, :, :
                ]
