...
class EnhancedVQVAE(nn.Module):
    def __init__(
        ...
        decay=0.99,
        quantize=True,
        beta=1.0,  # For beta-VAE extension
    ):
        super().__init__()
        ...
        # For potential beta-VAE extension
    ...
    def encode_to_distribution(self, input):
        """
        For beta-VAE extension: encode input to latent distribution parameters
        """
        # This would be used in a beta-VAE variant
        # For now, we just return the input as mean with zero variance
        batch_size, t, num_patches, emb_dim = input.shape
        input_flat = input.contiguous().view(-1, emb_dim)
        mu = input_flat
