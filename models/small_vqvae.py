...
class Encoder(nn.Module):
    def __init__(
        self, in_channel, channel, n_res_block, n_res_channel, stride
    ):
        super().__init__()
...
class Decoder(nn.Module):
    def __init__(
        ...
        if stride == 4:
            ...
            # Three upsampling stages for 8x8 -> 16x16 -> 32x32 -> 64x64
            blocks.extend(
                [
                    nn.ConvTranspose2d(
                        channel, channel // 2, 4, stride=2, padding=1
            ...
            # No upsampling, just channel conversion
            blocks.append(nn.Conv2d(channel, out_channel, 3, padding=1))
...
class SmallVQVAE(nn.Module):
    def __init__(
        self,
        in_channel=3,
        channel=64,  # Reduced from 128 for 64x64 images
        n_res_block=1,  # Reduced from 2 for 64x64 images
        n_res_channel=32,
        emb_dim=256,  # Match ResNetSmallTokens output
        n_embed=512,
        decay=0.99,
        quantize=False,  # Disable quantization for simplicity
        stride=2,  # Changed to 2 for 8x8 -> 64x64 upsampling
    ):
        super().__init__()
        ...
        # Single decoder for 64x64 images - much simpler
    ...
    def decode_code(
        self, code_b
    ):  # not used (only used in sample.py in original repo)
